   ...
   runningInCordova = true;
    ...
    function login(callback, options) {
        ...
        loginWindow = window.open(loginURL + '?client_id=' + fbAppId + '&auth_type=rerequest&redirect_uri=' + redirectURL +
            '&response_type=token&scope=' + scope, '_blank', 'location=no,clearcache=no,toolbar=no');
    ...
    function logout(callback) {
        ...
        if (token) {
            logoutWindow = window.open(logoutURL + '?access_token=' + token + '&next=' + logoutRedirectURL, '_blank', 'toolbar=no,location=no,clearcache=yes');
            if (runningInCordova) {
                setTimeout(function() {
                    logoutWindow.close();
                }, 1);
    ...
    function api(obj) {
        ...
        url = 'https://graph.facebook.com/v2.4/' + obj.path + '?' + toQueryString(params);
