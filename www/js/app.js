...
$$(document).on("focus","input,textarea", function(e){ alert('focus');$$(document).on("focus",".kbdfix", function(e){ var el = $$(e.target); var page = el.closest(".page-content"); var elTop = el.offset().top; //do correction if input at near or below middle of screen if(elTop > page.height() / 2 - 20 ){ var delta = page.offset().top + elTop - $$(".statusbar-overlay").height() * (myApp.device.ios?2:1) - $$(".navbar").height(); //minus navbar height?&quest;? 56 fot MD var kbdfix = page.find("#keyboard-fix"); if(kbdfix.length == 0) { //create kbdfix element page.append("<div id='keyboard-fix'></div>"); } $$("#keyboard-fix").css("height", delta * 2 + "px"); page.scrollTop( delta, 300); } }, true); //$$(document).on("blur","input,textarea", function(e){ //call this code in the Back button handler - when it fired for keyboard hidding. $$(document).on("blur",".kbdfix", function(e){ //reduce all fixes $$("#keyboard-fix").css("height", "0px"); }, true); ---
...
'<div class="swiper-container swiper-questions" style="height:100%;overflow-y:scroll;"  onscroll="scrollQuestions();">'+
 ...
 '<div style="background-color:transparent;height:100%;width:100%;overflow-x:hidden;" onclick="backtoProfile();">'+
...
'</div>',
  ...
  ' <div class="toolbar-inner duck-template" style="display:none;padding:0;background-color:#2196f3;">'+
   ...
   ' <div class="toolbar-inner toolbardecide" style="padding:0;">'+
